...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>  <%--jstl函数库--%>
                                ...
                                <c:forEach items="${roleList}" var="role" varStatus="vs">
                                     <span style="padding:3px;margin-right:30px;width: 160px;display: inline-block">
                                         <input type="checkbox"
                                                name="roleIds"
                                                value="${role.id}"
                                                ${fn:contains(userRoles, role.id) ? "checked":""}/>
                                                <%--${fn:contains(name, searchString)} jstl的函数库 大部分是通用的字符串处理函数--%>
                                         ${role.name}
                                     </span>
                                </c:forEach>
                            </div>
